        ...
        <form method="POST" action="">
            ...
            for ($i = 0; $i <= $num_encargos; $i++) {
                ...
                <fieldset>
                    ...
                    <input type="date" id="fecha_<?php echo $i; ?>" name="fecha_<?php echo $i; ?>"
                        value="<?php echo $encargo['fecha'] ?>"
                    ...
                    <select name="almacen_<?php echo $i; ?>" id="almacen_<?php echo $i; ?>"
                        ...
                        <option value="Aire libre" <?php echo ($encargo['almacen'] === 'Aire libre') ? 'selected' : ''; ?>>Aire
                            libre</option>
                        <option value="Interior" <?php echo ($encargo['almacen'] === 'Interior') ? 'selected' : ''; ?>>Interior
                        </option>
                        <option value="En frio" <?php echo ($encargo['almacen'] === 'En frio') ? 'selected' : ''; ?>>En frio
                        </option>
                    </select><br><br>

                    <label for="precio_<?php echo $i; ?>">Precio estimado (€):</label>
                    <input type="number" id="precio_<?php echo $i; ?>" name="precio_<?php echo $i; ?>" step="0.01" min="1.00"
                        max="999.99" value="<?php echo $encargo['precio']; ?>"
                    ...
                    <textarea id="descripcion_<?php echo $i; ?>" name="descripcion_<?php echo $i; ?>"
                        class="<?php echo isset($errores["descripcion_$i"]) ? 'campo-invalido' : ''; ?>" maxlength="500"
                        required><?php echo $encargo['descripcion']; ?>
                                                </textarea><br><br>
                </fieldset>
            <?php } ?>
