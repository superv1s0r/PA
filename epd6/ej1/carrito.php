            ...
            if ($accion === 'agregar') {
                ...
                foreach ($_SESSION['carrito'] as $producto) {
                    if ($producto['operacion'] === 'crear') {
                        ...
                        $stmt->execute([$producto['descripcion'], $producto['num_pasillo'], $producto['num_estanteria'], $producto['cantidad']]);
                    } elseif ($producto['operacion'] === 'modificar') {
                        $query = "UPDATE producto SET descripcion=?, num_pasillo=?, num_estanteria=?, cantidad=? WHERE sku=?";
                        $stmt = $conexion->prepare($query);
                        $stmt->execute([$producto['descripcion'], $producto['num_pasillo'], $producto['num_estanteria'], $producto['cantidad'], $producto['sku']]);
                    }
                }
                $_SESSION['carrito'] = [];
...
<body>
    <div class="container">
        ...
        <form action="carrito.php" method="POST">
            ...
            <button type="submit" name="accion" value="agregar" style="margin: 5px">Agregar</button>
            <button type="submit" name="accion" value="modificar" style="margin: 5px">Modificar</button>
        </form>
